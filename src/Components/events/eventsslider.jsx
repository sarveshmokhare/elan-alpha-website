...
export default function EventSlider({ imagesList }) {

  const [HoverValue, setHoverValue] = useState(false)

  const style = {
    width: (imagesList.length * 50).toString() + '%',
  ...
  return (
    <div style={style} onMouseEnter={() => { setHoverValue(true) }} onMouseLeave={() => { setHoverValue(false) }}>
      ...
      <div className="flex w-1/2">
        {imagesList.map((item) => (
          <div className="m-2 object-cover w-1/2 h-[15vw] overflow-hidden"><img src={item} alt="" className="h-full w-full" /></div>
        ))}
      </div>
      ...
      <div className="flex w-1/2">
        {imagesList.map((item) => (
          <div className="object-cover m-2 w-1/2 h-[15vw] overflow-hidden">
