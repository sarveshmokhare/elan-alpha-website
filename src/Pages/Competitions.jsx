...
function ShowDetails({ item }) {
  ...
  return (
    <div>
      <Popup trigger={
        <button className="bg-golden text-black rounded-md text-md px-3 font-century m-1 lg:leading-loose">Learn more</button>
          ...
          <div className='bg-blue w-full h-full px-10 overflow-auto z-5 relative bg-[url("assets/designs/competitions_popup_2.svg"),url("assets/designs/competitions_popup_3.svg"),url("assets/designs/competitions_popup_4.svg"),url("assets/designs/competitions_popup_1.svg"),url("assets/designs/competitions_popup_5.svg")] bg-no-repeat bg-[position:left_top,left_bottom,right_bottom,left_center,right_center] bg-[length:150px,100px,50px,100px,200px] rounded-xl'>
            ...
            <div className='flex flex-row  justify-evenly mb-5 flex-wrap'>
              {popupnav.map((item, index) => (
                <button onClick={() => { setpopupNavValue(index + 1) }} className={'block text-xl p-1 m-2 ' + (popupNavValue == (index + 1) ? 'text-black bg-golden' : 'text-golden bg-blue ')}>{item}</button>
              ))}
            </div>
            <div className={'flex relative flex-col md:flex-row ' + (popupNavValue == 1 ? 'block' : 'hidden')}>
              <div className='w-full md:w-2/3 flex-col justify-evenly text-golden mx-1'>
                <div className='text-2xl'>{item.details}</div>
                <button className="bg-golden text-black rounded-lg px-3 font-century m-1 relative">Register</button>
              </div>
              <img src={item.img} className='w-full md:w-1/3 mx-1 object-cover p-5 md:p-0' alt="" />
            </div>

            <div className='m-5'>
              {popupnav.slice(1).map((item, index) => (
                <div className={'font-berkshire text-golden text-3xl  bg-[url("assets/designs/competitions_popup_1.svg")] bg-no-repeat mb-5 ' + (popupNavValue == (index + 2) ? ' block' : 'hidden')}>{item}</div>
              ))}


              <ul className={(popupNavValue == 2 ? ' block' : 'hidden')}>{item.rules.map((rules_item) => (
                <li className='text-golden'>{rules_item}</li>
              ))}
              </ul>

              <ul className={(popupNavValue == 3 ? ' block' : 'hidden')}>
                <li>
                  <div className='text-golden text-3xl inline'>Submission deadline:</div>
                  <div className='text-golden text-3xl inline'>{item.timeline.submission}</div>
                </li>
                <li>
                  <div className='text-golden text-3xl inline'>Event date</div>
                  <div className='text-golden text-3xl inline'>{item.timeline.event}</div>
                </li>
              </ul>

              <ul className={(popupNavValue == 4 ? ' block' : ' hidden')}>
                {item.judging.map((judging_item)=>(
                  <li className='text-golden text-xl'>{judging_item}</li>
                ))}
              </ul>
              <ul className={(popupNavValue == 5 ? ' block' : ' hidden')}>
                {item.prizes.map((prizes_item) => (
                  <li className='text-golden text-xl'>{prizes_item}</li>
                ))}
              </ul>
              <ul className={'list-disc text-golden'+(popupNavValue == 6 ? ' block' : ' hidden')}>
                {item.contact.map((contact_item) => (
                  <li className='text-golden text-xl flex'>
    ...
    <div className='z-0'>
      <div className='flex lg:justify-center justify-evenly pb-10'>
        {navbuttons.map((nav_item,index)=>(
          <button onClick={() => setNavigateValue(index)} className={"rounded px-8 mx-5 text-2xl  md:text-3xl md:leading-loose lg:leading-normal lg:text-3xl " + (Navigatevalue == index ? "bg-golden text-black" : "bg-blue text-golden")} >{nav_item}</button>
        ))}
      </div>

      <div className='lg:hidden'>
        <Slider imagesList={Navigatevalue == 1 ? cultidata.map((item) => item.img) : techydata.map((item) => item.img)} />
      </div>
...
function Competitions() {
  ...
  return (
    <div className={context.theme + " bg-black bg-no-repeat p-10 bg-[position:left_top_-5rem_,_right_bottom_-4.5rem] md:bg-[length:400px,200px] bg-[length:50%,10%] bg-[url('./assets/designs/competitions_mobile_1.svg'),url('./assets/designs/competitions_mobile_2.svg')] lg:bg-[url('./assets/designs/competitions_desktop_1.svg')] lg:bg-[right_top] lg:bg-[length:100px] "}>
